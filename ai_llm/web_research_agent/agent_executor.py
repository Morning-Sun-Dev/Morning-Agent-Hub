...
class WebResearchAgentExecutor(AgentExecutor):
    ...
    async def execute(
        ...
        try:
            async for item in self.agent.stream(query):
                ...
                if not is_complete and not require_input:
                    ...
                    if sources:
                        ...
                        await updater.add_artifact(
                            parts=[Part(root=TextPart(text=sources_json))],
            ...
            await updater.update_status(
                TaskState.failed,
                new_agent_text_message(f"오류 발생: {str(e)}", task.context_id, task.id),
            )
    ...
    async def cancel(
        ...
        await updater.cancel(
            new_agent_text_message("웹 검색 작업이 취소되었습니다.", task.context_id, task.id)
        )
